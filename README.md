...
The patched go2rtc **source and build** live in a separate fork so the git history and upstream attribution are preserved: **[github.com/ajplotkin/go2rtc](https://github.com/ajplotkin/go2rtc/tree/nestfix-1.9.14-17)** — build from the stable tag **`nestfix-1.9.14-17`** (the tags are a branchless chain on top of upstream master, so build from the tag — there is no branch to check out.). Part 3 shows how to build it. This work also folds in several community go2rtc pull requests, credited at the end.
...
cd go2rtc
git checkout nestfix-1.9.14-17   # stable tag — not the dev branch
```
...
had moved to `nestfix-1.9.14-17` and this README still pinned `-11`.
