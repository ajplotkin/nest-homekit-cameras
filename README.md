...
The patched go2rtc **source and build** live in a separate fork so the git history and upstream attribution are preserved: **[github.com/ajplotkin/go2rtc](https://github.com/ajplotkin/go2rtc/tree/nestfix-1.9.14-16)** — build from the stable tag **`nestfix-1.9.14-16`** (the tags are a branchless chain on top of upstream master, so build from the tag — there is no branch to check out.). Part 3 shows how to build it. This work also folds in several community go2rtc pull requests, credited at the end.
...
Most of Google Device Access (Part 1) **can** be scripted, which this guide used to deny. [`scripts/setup-google-device-access.sh`](scripts/setup-google-device-access.sh) automates the Cloud project, the SDM and Pub/Sub APIs, the topic, the `sdm-publisher` IAM binding, the pull subscription and the token exchange. Only three things genuinely need a browser and a human: the **$5 Device Access registration** (it takes a payment), the **OAuth consent screen and client**, and **approving the authorization URL**. The script walks you to each page and waits. The approach is [@WeekendSuperhero's](https://github.com/WeekendSuperhero), from [potmat/homebridge-google-nest-sdm#209](https://github.com/potmat/homebridge-google-nest-sdm/pull/209). Once you have those credentials in a working **Homebridge + homebridge-google-nest-sdm 1.1.24** install, the rest is one script.
...
cd go2rtc
git checkout nestfix-1.9.14-16   # stable tag — not the dev branch
```
...
> **`PrebufferManager.js` is the exception to all of the above** — it is a whole file, not a diff, so there is nothing to re-cut and `patch` will never warn you that it has gone stale. On a new plugin release, check two things by hand: that `StreamingDelegate.js` still calls it the same way, and that upstream has not shipped its own prebuffer (see [#240](https://github.com/potmat/homebridge-google-nest-sdm/pull/240)) — if it has, drop ours rather than layering them. Then run `node test/prebuffer-noise.test.js`, which asserts the retry logging stays throttled both for a camera that was already off at startup and for one switched off after it had been streaming. It fakes ffmpeg, the clock and the timers, so it needs nothing running and finishes in about a second; it exits non-zero on failure. (The other automated checks here are [`scripts/check-drift.sh`](scripts/check-drift.sh), which compares the Pi against this repo, and [`scripts/test-prebuffer-e2e.js`](scripts/test-prebuffer-e2e.js), which needs a live restreamer.)
...
reads `WRONG BUILD` and names both revisions. Adding `--deep` also verifies that all 15 files
...
had moved to `nestfix-1.9.14-16` and this README still pinned `-11`.
